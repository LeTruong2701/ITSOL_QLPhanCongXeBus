package view;
import Entity.BusManagement;
import Entity.BusRoute;
import Ultil.ControllerUltility;
import Ultil.Data;
...
public class View {
    ...
    public static void main(String[] args) {
        ...
        do {
            System.out.println("\n____________Quản lý phân công lái xe____________");
            System.out.println("1. Thêm lái xe.");
            System.out.println("2. Hiển thị danh sách lái xe.");
            System.out.println("3. Thêm 1 tuyến đường.");
            ...
            switch (choice) {
                ...
                case 2:
                    drivers = readDriverInfo(data, "LAIXE.DAT");
                    showDriverInfo(drivers);
                    break;

                case 3:
                    getBusRouteInfo(data);
                    break;

                case 4:
                    busRoute = readRouteInfo(data, "TUYENDUONG.DAT");
                ...
                case 5:
                    ...
                    managerments = data.readBMFromFile("BM.DAT");
                    managerments = update(controllerUltility, managerments, data);
                    boolean done = false;
                    do {
                        showDriverInfo(drivers);
                    ...
                    if (done) {
                        ...
                        BusManagement b = new BusManagement(driver, busRoute1,
                                numOfDriverdRoute);
                        data.writeBMToFile(b, "BM.DAT");
                    } else {
                        System.out.println("Lái xe đã lái đủ  số lượng cho phép!");
                    }
                    managerments = readBMInfo(data);
                    managerments = update(controllerUltility, managerments, data);
    ...
    private static void updateId(Data data) {
        Driver[] drivers = readDriverInfo(data, "LAIXE.DAT");
        if(drivers.length > 0){
            Driver.id = drivers[drivers.length - 1].getDriverID()+1;
        }

        BusRoute[] busRoutes = readRouteInfo(data, "TUYENDUONG.DAT");
    ...
    private static BusManagement[] update(ControllerUltility controllerUltility,
        ...
        Driver[] drivers = readDriverInfo(data, "LAIXE.DAT");
        BusRoute[] busRoutes = readRouteInfo(data, "TUYENDUONG.DAT");
        return controllerUltility.updateBRMInfo(managerments, drivers, busRoutes);
    }

    private static BusManagement[] readBMInfo(Data data) {
        return data.readBMFromFile("BM.DAT");
    }
    ...
    private static void getBusRouteInfo(Data data) {
        ...
        data.writeRouteToFile(busRoute, "TUYENDUONG.DAT");
    }
    ...
    private static void getDriverInfo(Data data) {
        ...
        data.writeDriverToFile(driver, "LAIXE.DAT");
    }

    private static Driver[] readDriverInfo(Data data, String s) {
        return data.readAllDriverFromFile(s);
    }

    private static BusRoute[] readRouteInfo(Data data, String s) {
        return data.readAllRouteFromFile(s);
    }
}
